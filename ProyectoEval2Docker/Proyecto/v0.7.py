...
class App:
    def __init__(self, root):
        ...
        self.mostrar_btn = ttk.Button(root, text="Mostrar Reservas", command=self.mostrar_reservas)
        self.mostrar_btn.pack(pady=5, padx=20, fill=tk.X)

        self.agregar_btn = ttk.Button(root, text="Agregar Reserva", command=self.agregar_reserva)
        self.agregar_btn.pack(pady=5, padx=20, fill=tk.X)

        self.cancelar_btn = ttk.Button(root, text="Cancelar Reserva", command=self.cancelar_reserva)
        self.cancelar_btn.pack(pady=5, padx=20, fill=tk.X)

        self.salir_btn = ttk.Button(root, text="Salir", command=self.salir)
    ...
    def agregar_reserva(self):
        ...
        tk.Label(self.ventana_agregar, text="Fecha (DD/MM/AAAA):", bg="#f0f0f0").grid(row=0, column=0, pady=5, padx=10, sticky=tk.W)
        self.fecha_entry = tk.Entry(self.ventana_agregar)
        self.fecha_entry.grid(row=0, column=1, pady=5, padx=10)

        tk.Label(self.ventana_agregar, text="Hora (HH:MM):", bg="#f0f0f0").grid(row=1, column=0, pady=5, padx=10, sticky=tk.W)
        self.hora_entry = tk.Entry(self.ventana_agregar)
        self.hora_entry.grid(row=1, column=1, pady=5, padx=10)

        tk.Label(self.ventana_agregar, text="Nombre:", bg="#f0f0f0").grid(row=2, column=0, pady=5, padx=10, sticky=tk.W)
        self.nombre_entry = tk.Entry(self.ventana_agregar)
        self.nombre_entry.grid(row=2, column=1, pady=5, padx=10)

        tk.Button(self.ventana_agregar, text="Agregar", command=self.confirmar_agregar, bg="#2196F3", fg="white").grid(row=3, column=0, pady=10, padx=10)
        tk.Button(self.ventana_agregar, text="Limpiar", command=self.limpiar_campos, bg="#FFC107").grid(row=3, column=1, pady=10, padx=10)
    ...
    def cancelar_reserva(self):
        ...
        tk.Label(self.ventana_cancelar, text="Fecha (DD/MM/AAAA):", bg="#f0f0f0").grid(row=0, column=0, pady=5, padx=10, sticky=tk.W)
        self.fecha_cancelar_entry = tk.Entry(self.ventana_cancelar)
        self.fecha_cancelar_entry.grid(row=0, column=1, pady=5, padx=10)

        tk.Label(self.ventana_cancelar, text="Hora (HH:MM):", bg="#f0f0f0").grid(row=1, column=0, pady=5, padx=10, sticky=tk.W)
        self.hora_cancelar_entry = tk.Entry(self.ventana_cancelar)
        self.hora_cancelar_entry.grid(row=1, column=1, pady=5, padx=10)

        tk.Button(self.ventana_cancelar, text="Cancelar", command=self.confirmar_cancelar, bg="#f44336", fg="white").grid(row=2, columnspan=2, pady=10)
    ...
    def confirmar_cancelar(self):
        ...
        messagebox.showinfo("Resultado", resultado)
        self.fecha_cancelar_entry.delete(0, tk.END)
        self.hora_cancelar_entry.delete(0, tk.END)
